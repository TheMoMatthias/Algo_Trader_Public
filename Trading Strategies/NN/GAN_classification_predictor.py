...
class LSTMGANModelTrainer:
    ...
    def compile_gan(self, params):
        ...
        self.generator = self.build_generator(input_shape, lstm_units, dropout_rate, kernel_regularizer)
        self.discriminator = self.build_discriminator(input_shape, dropout_rate, kernel_regularizer)
        ...
        self.gan.compile(optimizer=self.generator_optimizer, loss='binary_crossentropy', metrics=['accuracy'])
    ...
    def train_generator(self, real_data):
        ...
        with tf.GradientTape() as tape:
            generated_data = self.generator(real_data, training=True)
            g_loss = self.loss_fn(tf.ones_like(self.discriminator(generated_data, training=True)), self.discriminator(generated_data, training=True))
    ...
    def train(self):
        ...
        y = self.predicting_variable_adj.values
    ...
    def save_model(self):
        model_filename = os.path.join(os.path.dirname(__file__), 'models', 'gan_generator.h5')
        self.generator.save(model_filename)
        discriminator_filename = os.path.join(os.path.dirname(__file__), 'models', 'gan_discriminator.h5')
