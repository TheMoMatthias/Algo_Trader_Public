...
from Base_NN_skeletons import convolutional_lstm_gan_classification as nn_architecture   #convolutional_lstm_gan_classification lstm_gan_classification
import psutil
...
class LSTMGANModelTrainer:
    ...
    def compile_gan(self, params):
        ...
        discriminator_input_shape = (self.window_size, 3)  # The generator's output shape
        ...
        self.generator_optimizer = Adam(learning_rate=learning_rate_gen, beta_1=beta_1, clipvalue=1.0)
        self.discriminator_optimizer = Adam(learning_rate=learning_rate_disc, beta_1=beta_1, clipvalue=1.0)
        ...
        self.gan.compile(optimizer=self.generator_optimizer, loss='sparse_categorical_crossentropy', metrics=['accuracy'])

        self.discriminator.trainable = True
        
    @tf.function
    def train_discriminator(self, real_data, real_labels, real_labels_vector):
        ...
        with tf.GradientTape() as tape:
            ...
            # Add noise to the real and generated data to increase difficulty
            noise_factor = 0.05
            ...
            # real_data_shaped = tf.reshape(real_data_shaped, [-1, self.window_size, 3])

            d_real_pred = self.discriminator(real_data_noisy, training=True)
            d_fake_pred = self.discriminator(generated_data_noisy, training=True)

            # Real labels are ones and fake labels are zeros
            real_targets = tf.ones_like(d_real_pred)
            fake_targets = tf.zeros_like(d_fake_pred)
            
            # Apply class weights
            weights = tf.gather([self.class_weights[i] for i in range(len(self.class_weights))], real_labels)

            d_real_loss = tf.keras.losses.binary_crossentropy(real_targets, d_real_pred) *weights   #comment *weights out if you do not want to use class weights
            d_fake_loss = tf.keras.losses.binary_crossentropy(fake_targets, d_fake_pred)
    ...
    @tf.function
    def train_generator(self, real_data, real_labels, real_labels_vector):
        real_data = tf.cast(real_data, tf.float32)
        real_labels = tf.cast(real_labels, tf.int32)
        real_labels_vector = tf.cast(real_labels_vector, tf.float32)
        ...
        with tf.GradientTape() as tape:
            generated_data = self.generator(real_data, training=True)
            # d_real_pred = self.discriminator(real_labels_vector, training=True)
            d_fake_pred = self.discriminator(generated_data, training=True)

            # Real labels are ones for the generator loss
            real_targets = tf.ones_like(d_fake_pred)
            adversarial_loss = tf.keras.losses.binary_crossentropy(real_targets, d_fake_pred)
            adversarial_loss = tf.reduce_mean(adversarial_loss)  # Reduce to scalar
            
            real_labels_vector_flat = tf.reshape(real_labels_vector, [-1, 3])
            generated_data_flat = tf.reshape(generated_data, [-1, 3])
            
            # Classification loss (sparse categorical cross-entropy) | Comment out if you do not want to use class weights
            ...
            class_weights = tf.reshape(class_weights, [-1, 1])  # Reshape to match the shape of real_labels_vector_flat
            classification_loss = tf.keras.losses.sparse_categorical_crossentropy(tf.argmax(real_labels_vector_flat, axis=1), generated_data_flat)
            classification_loss = classification_loss * tf.squeeze(class_weights)  # Apply class weights     #comment if you do not want to use class weights
            classification_loss = tf.reduce_mean(classification_loss)  # Reduce to scalar
            
            # g_loss = tf.keras.losses.binary_crossentropy(real_targets, d_fake_pred)
             # Combine losses
            g_loss = adversarial_loss + classification_loss
            
        if tf.math.reduce_any(tf.math.is_nan(g_loss)):
            print(f"g_loss contains NaN values")
    ...
    def train_gan(self, epochs, batch_size):
        ...
        for epoch in range(epochs):
            ...
            for start_idx in range(0, self.X_train.shape[0], batch_size):
                ...
                for _ in range(3):
                    # Train generator twice for each discriminator update
                    g_loss = self.train_generator(real_data, real_labels, real_labels_vector)
                    g_losses.append(g_loss)
                
                d_loss = self.train_discriminator(real_data, real_labels, real_labels_vector)
                d_losses.append(d_loss)

            avg_d_loss = np.mean(d_losses)
            avg_g_loss = np.mean(g_losses)

            custom_val_loss, custom_val_acc, val_loss, accuracy = self.evaluate(self.X_val, self.y_val, self.y_val_batched, self.y_val_vector, use_class_weights=False)
            y_pred_val = self.generator.predict(self.X_val)
            y_pred_val = tf.argmax(y_pred_val, axis=2, output_type=tf.int32) #tf.argmax(self.generator.predict(self.X_val), axis=1, output_type=tf.int32) # .numpy().flatten()
             # Extract the predicted class for the final time step in each window
    ...
    def train(self):
        ...
        val_split_idx = int(len(X)  * (1 - (self.test_size + self.val_size)))
    ...
    def evaluate(self, X, y_true, y_true_batched, y_true_vector, use_class_weights=False):
        ...
        if use_class_weights:
            class_weights = {i: self.class_weights[i] for i in range(len(self.class_weights))}
